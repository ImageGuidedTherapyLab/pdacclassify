...
if (options.predictimage != None and options.segmentation != None and options.c3dexe != None ):
  ...
  if ( options.lesionimage != None):
     resizepre = skimage.transform.resize(numpypredict[:,:,:,0,0],(options.trainingresample,options.trainingresample,nslice ),order=0,preserve_range=True,mode='constant').astype(IMG_DTYPE).transpose(2,1,0)
     resizeart = skimage.transform.resize(numpypredict[:,:,:,0,1],(options.trainingresample,options.trainingresample,nslice ),order=0,preserve_range=True,mode='constant').astype(IMG_DTYPE).transpose(2,1,0)
     resizeven = skimage.transform.resize(numpypredict[:,:,:,0,2],(options.trainingresample,options.trainingresample,nslice ),order=0,preserve_range=True,mode='constant').astype(IMG_DTYPE).transpose(2,1,0)
